...
func NewURLShortener(repo repository.URLRepository, cache cache.SyncableCache, metricsCollector metrics.Collector, generator shortener.Generator) URLShortener {
	return &urlShortener{
		repo:      repo,
		cache:     cache,
...
func (s *urlShortener) StartCacheSync(ctx context.Context, interval time.Duration) error {
	syncFunc := func(dirtyEntries map[string]*domain.CacheEntry) error {
		start := time.Now()
		s.metrics.IncrementCacheSyncOperations()
		
		entriesCount := len(dirtyEntries)
		s.metrics.RecordCacheSyncEntriesProcessed(entriesCount)
		
		for shortCode, entry := range dirtyEntries {
			if err := s.repo.UpdateUsage(ctx, shortCode, entry.UsageCount, entry.LastUsedAt); err != nil {
				s.metrics.IncrementCacheSyncErrors()
				return fmt.Errorf("failed to sync entry %s: %w", shortCode, err)
			}
		}
		
		s.metrics.RecordCacheSyncDuration(time.Since(start))
		return nil
	}
...
func (s *urlShortener) CreateShortURL(ctx context.Context, originalURL string) (*domain.URLEntry, error) {
	...
	if err != nil {
		s.metrics.IncrementURLsCreatedWithError("invalid_url")
	...
	if err != nil {
		s.metrics.IncrementURLsCreatedWithError("generator_error")
	...
	if err != nil {
		s.metrics.IncrementURLsCreatedWithError("database_error")
	...
	s.metrics.IncrementURLsCreated()
	return entry, nil
}
...
func (s *urlShortener) GetOriginalURL(ctx context.Context, shortCode string) (string, error) {
	...
	if entry, exists := s.cache.Get(ctx, shortCode); exists {
		...
		s.metrics.IncrementURLsRetrievedFromCache()
		...
		s.metrics.IncrementURLsRetrieved()
	...
	if err != nil {
		s.metrics.IncrementURLsRetrievedWithError("not_found")
		return "", fmt.Errorf("short code not found")
	}

	s.metrics.IncrementURLsRetrievedFromDB()
...
func (s *urlShortener) DeleteShortURL(ctx context.Context, shortCode string) error {
	...
	if err != nil {
		s.metrics.IncrementURLsDeletedWithError("database_error")
		return fmt.Errorf("failed to check URL existence: %w", err)
	}
	if !exists {
		s.metrics.IncrementURLsDeletedWithError("not_found")
		return fmt.Errorf("short code not found")
	}
	...
	s.metrics.IncrementURLsDeleted()
	return nil
}
